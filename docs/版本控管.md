...
# 3. 連結 GitHub 遠端倉庫
# 請替換為您的倉庫網址
git remote add origin https://github.com/AbauDog/dont_sleep.git   # 第一次
git pull origin main --allow-unrelated-histories   # 第一次
# 4. 首次上傳
git add .
git commit -m "專案首次上傳"
git commit -m "修正圖示"
git branch -M main    # 第一次

git push -u origin main

# -----狀態檢查
git fetch          # 從 GitHub 抓取最新的狀態資訊（但不影響目前程式碼）
git status         # 再次檢查狀態
若同步：顯示 up to date。
若落後（GitHub 有人更新）：顯示 Your branch is behind...。
若超前（本地有 commit 但沒 push）：顯示 Your branch is ahead of...。
